...
class FileManager:
    ...
    def __init__(self, base_path: Optional[str] = None, server_name: str = "brigado"):
        ...
        if base_path is None:
            ...
            base_path = current_file.parent.parent / "data"

        self.base_path = Path(base_path)
        self.server_name = server_name
        self.server_dir = self.base_path / server_name  # Server-specific directory

        # Reports directory at brigado_v2 level (shared across servers)
        self.reports_dir = self.base_path.parent / "reports"
        self.html_dir = self.base_path.parent / "reports"

        # Server-specific directories
        self.live_databases_dir = self.server_dir / "live_databases"
        self.data_sources_dir = self.server_dir  # Consolidated parquets live here

        # Legacy directories (deprecated, kept for backwards compatibility)
    ...
    def list_servers(self) -> List[str]:
        """
        List all server directories in the data folder.

        Returns:
            List of server names (directory names in data/)
        """
        servers = []
        ...
        for item in self.base_path.iterdir():
            if item.is_dir() and not item.name.startswith('.') and item.name not in ['enriched', 'exports']:
